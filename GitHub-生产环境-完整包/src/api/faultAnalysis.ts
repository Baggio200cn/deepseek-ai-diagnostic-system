...
export const analyzeText = async (text: string): Promise<TextAnalysisResponse> => {
  try {
    ...
    const getApiEndpoint = () => {
      // 检查是否有真实的DeepSeek API配置
      const hasRealApiKey = process.env.REACT_APP_DEEPSEEK_API_KEY && 
                           process.env.REACT_APP_DEEPSEEK_API_KEY !== 'your-deepseek-api-key-here';
      
      // 如果有真实API密钥且在生产环境，尝试使用真实API
      if (hasRealApiKey && (window.location.hostname.includes('github.io') || 
          window.location.hostname.includes('vercel.app'))) {
        return 'https://api.deepseek.com/v1/chat/completions'; // DeepSeek官方API
      }
      
      // 否则使用本地智能模拟（包括GitHub Pages部署）
      return null; // 标记为智能模拟模式
    };

    const apiEndpoint = getApiEndpoint();
    
    // 智能模拟模式：提供高质量的专业诊断（适用于本地开发和GitHub Pages）
    if (!apiEndpoint) {
    ...
    if (data.success && data.analysis) {
      return {
        analysis: {
          faultType: data.analysis.faultType || '智能诊断分析',
          confidence: data.analysis.confidence || 0.92,
          solutions: Array.isArray(data.analysis.solutions) ? 
            data.analysis.solutions : [data.analysis.solutions || '请联系专业技术人员'],
          description: data.analysis.description || '基于DeepSeek AI的专业故障诊断',
    ...
    if (data.fallback && data.analysis) {
      return {
        analysis: {
          faultType: data.analysis.faultType || '智能备用诊断',
          confidence: data.analysis.confidence || 0.75,
          solutions: Array.isArray(data.analysis.solutions) ? 
            data.analysis.solutions : [data.analysis.solutions || '请提供更详细信息'],
          description: data.analysis.description || '基于智能算法的备用诊断',
          severity: data.analysis.severity || 'medium'
        },
        statistics: {
          totalFrames: 1,
